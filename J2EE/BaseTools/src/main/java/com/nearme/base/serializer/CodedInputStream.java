...
/**
 * ClassName:CodedInputStream <br>
 * Function: 对Nearme格式的数据进行解码，改编自google protobuf <br>
 *
 * @author   80036381
 * @version
 * @since    Ver 1.1
 * @Date	 2013-5-15  下午2:58:06
 */
public final class CodedInputStream {
	...
	public static CodedInputStream newInstance(final byte[] buf, final int off,
		...
		} catch (LazySerializeException ex) {
	...
	public int readTag() throws IOException {
		...
		if (WireFormat.getTagFieldNumber(lastTag) == 0) {
			...
			throw LazySerializeException.invalidTag();
		}
		return lastTag;
	}
	...
	// throw LazySerializeException.recursionLimitExceeded();
	// }
	// ++recursionDepth;
	...
	public void readMessage(ILazySerialize message) throws IOException {
	...
	/** Read an embedded repeat field value from the stream. */
	public List<?> readRepeat() throws IOException {
		int listSize = readInt32();
		List<Object> list = new ArrayList<Object>(listSize);
		if (listSize > 0) {
			int type = readInt32();
			LazySerializeType lazySerializeType = LazySerializeTypeHelper.getFromType(type);
			boolean isMessage = LazySerializeType.TYPE_MESSAGE == lazySerializeType;
			if (isMessage) {
				//repeat中的message需要对元数据进行处理，避免每个message需要一个元数据
				ILazyMessage firstMessage = LazySerializeFactory.createLazyMessage();
				readMessage(firstMessage);
				list.add(firstMessage);

				for (int i = 1; i < listSize; i++) {
					ILazyMessage message = LazySerializeFactory.createLazyMessage();
					message.setMetaData(firstMessage.getMetaData());
					readMessage(message);
					list.add(message);
				}
			} else {
				for (int i = 0; i < listSize; i++) {
					Object obj = readObject(lazySerializeType);
					if (null != obj) {
						list.add(obj);
					}
				}
...
//			case TYPE_MESSAGE:
//				for (int i = 0; i < listSize; i++) {
//					ILazySerialize message = LazySerializeFactory.createDefaultLazySerialize();
//					list.add(message);
//				}
//
//				break;
...
//			}
	 ...
	 * @return
	 */
	public Object readObject(LazySerializeType lazySerializeType) throws IOException {
		switch(lazySerializeType) {
		case TYPE_INT:
			return readInt32();
		case TYPE_LONG:
			...
			if ((tmp = readRawByte()) >= 0) {
				...
				if ((tmp = readRawByte()) >= 0) {
					...
					if (tmp < 0) {
						...
						throw LazySerializeException.malformedVarint();
					}
				}
			}
		}
	...
	static int readRawVarint32(final InputStream input) throws IOException {
		...
		if (firstByte == -1) {
			throw LazySerializeException.truncatedMessage();
		}
	 ...
	 * @return the old limit.
	 */
	public int pushLimit(int byteLimit) throws LazySerializeException {
		if (byteLimit < 0) {
			throw LazySerializeException.negativeSize();
		}
	...
	private boolean refillBuffer(final boolean mustSucceed) throws IOException {
		...
		if (bufferSize == -1) {
			...
			if (totalBytesRead > sizeLimit || totalBytesRead < 0) {
				throw LazySerializeException.sizeLimitExceeded();
			}
			return true;
		}
	}
