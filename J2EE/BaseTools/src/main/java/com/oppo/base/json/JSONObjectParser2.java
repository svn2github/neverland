...
/**
 * ClassName:JSONObjectParser2 <br>
 * Function: 将一个独立的xml节点解析为对象的工具，支持 <br>
 *
 * @author   80036381
 * @version
 * @since    Ver 1.1
 * @Date	 2012-11-29  下午05:01:49
 */
public class JSONObjectParser2 {
	private INameTransfer nameTransfer;
	private IValueTransfer<String> strValueTransfer;
	 ...
	 */
	public <T> T getObjectFromJSON(String json, Class<T> tClass, Map<String, Class<?>> dClassMap) throws Exception {
		if (StringUtil.isNullOrEmpty(json)) {
			return null;
		}

		return parseJSON(new JSONObject(json), tClass, dClassMap);
	}
	 ...
	 * @param tClass 数据实体类型
	 */
	public <T> List<T> getListFromJSON(String json, Class<T> tClass, Map<String, Class<?>> dClassMap) throws Exception {
		if (StringUtil.isNullOrEmpty(json)) {
			return null;
		}
		...
		for (int i = 0; i < length; i++) {
			T t = parseJSON(array.getJSONObject(i), tClass, dClassMap);
			if (null != t) {
				tList.add(t);
			}
		}
	 ...
	 * @throws Exception
	 */
	public <T> T parseJSON(JSONObject jsonObject, Class<T> tClass,
			Map<String, Class<?>> dClassMap) throws Exception {
		if (null == jsonObject) {
			return null;
		}
	 ...
	 * @return
	 */
	protected <T> boolean handleArrayProperty(String name,
			JSONArray jsonArray, T tObj, Map<String, Class<?>> dClassMap) throws Exception {
		String methodName = getNameTransfer().getGetMethod(name);
		Method m = MethodsUtil.getMethod(tObj.getClass(), methodName);
		if (List.class.isAssignableFrom(m.getReturnType())) {
			Class<?> dClass = null == dClassMap ? null : dClassMap.get(name);
			if(null == dClass) {
				return false;
			}

			int len = jsonArray.length();
			List<Object> propList = new ArrayList<Object>(len);
			for (int i = 0; i < len; i++) {
				JSONObject jsonObj = (JSONObject) jsonArray.get(i);
				Object dObj = this.parseJSON(jsonObj, dClass, dClassMap);
				if (null != dObj) {
					propList.add(dObj);
				}
			}
	...
	public IValueTransfer<String> getStrValueTransfer() {
		if (null == strValueTransfer) {
			strValueTransfer = new JSONValueTransfer(null);
		}
	 ...
	 * @since Ver 1.0
	 */
	public void setStrValueTransfer(IValueTransfer<String> strValueTransfer) {
