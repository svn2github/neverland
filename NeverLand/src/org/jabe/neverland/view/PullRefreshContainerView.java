 ...
 * using {@link #setList(ListView)} and {@link #setRefreshHeader(View)}
 * <p>
...
public class PullRefreshContainerView extends LinearLayout {
	...
	private int mCurRefreshViewHeight = 50;
	...
	private int REFRESH_VIEW_HEIGHT = 50;
	...
	private void changeState(int nowState) {
		switch (nowState) {
		case STATE_IDLE:
		...
		case MotionEvent.ACTION_MOVE:
			if (mList.getFirstVisiblePosition() == 0
					&& (mList.getChildCount() == 0 || mList.getChildAt(0)
							.getTop() == 0)) {
				if ((mInterceptY - oldLastY > 0) || (mState == STATE_PULL)
	 ...
	 */
	private void notifyStateChanged(int nowState) {
		if (mOnChangeStateListener != null) {
			mOnChangeStateListener.onChangeState(this, nowState, mState);
			mState = nowState;
		}
	}
